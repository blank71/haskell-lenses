...
firstAndLast (Time t tm) = Just $ upd $ firstAndLast tm
  where
    upd Nothing = (t, t)
    upd (Just (_, t1)) = (t, t1)
firstAndLast (Split tm1 tm2) = pick (firstAndLast tm1) (firstAndLast tm2)
  where
    pick Nothing t = t
    pick t Nothing = t
...
diffToMs (t1, t2) = round $ toRational df * 1e3
  where
