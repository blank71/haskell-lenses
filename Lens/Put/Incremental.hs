...
put_delta_join_left c (l1 :: Lens s1) (l2 :: Lens s2) (_ :: Lens s) delta_o =
  do
    ...
    let delta_m' =
          delta_m0
            #- ( project @(VarsEnv (Rt s1)) $ positive delta_l,
  ...
  where
    ...
    pjoin ::
      (Project (InterCols (Rt s1) (Rt s2)) (Rt sl), ToDynamic (ProjectEnv (InterCols (Rt s1) (Rt s2)) (Rt sl))) =>
      Lens sl ->
      RecordsDelta (Rt sl) ->
      DPhrase
    pjoin (l :: Lens sl) delta =
      DP.conjunction
        [ P.In
            (recover @(InterCols (Rt s1) (Rt s2)) Proxy)
...
put_delta_join_templ c delfn (l1 :: Lens s1) (l2 :: Lens s2) (l :: Lens s) delta_o =
  ...
  where
    ...
    pjoin ::
      (Project (InterCols (Rt s1) (Rt s2)) (Rt sl), ToDynamic (ProjectEnv joincols (Rt sl))) =>
      Lens sl ->
      RecordsDelta (Rt sl) ->
      DPhrase
    pjoin (l :: Lens sl) delta =
